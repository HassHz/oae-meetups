...
var endMeetup = module.exports.endMeetup = function(ctx, groupId, callback) {
    var validator = new Validator();
    ...
    PrincipalsAPI.getFullGroupProfile(ctx, groupId, function(err, groupProfile) {
        ...
        if (!profile.isManager) {
            return callback(null, {responsecode: 'failed', response: 'Only group managers may end meetups'});
        }

        MeetupsAPI.Bbb.endURL(ctx, profile, function(err, endInfo) {
            if(err) {
                //res.send(503, 'Fatal error'); *res is not defined here...*
                log().info('Fatal error');
            }
            ...
            if (endInfo.url) {
                BBBProxy.executeBBBCall(endInfo.url, function (err, endInfo) {
                    if (err) {
                        return callback(err);
                    }
                    return callback(null, endInfo);
                });
            }
...
var deleteRecordingLink = module.exports.deleteRecordingLink = function(ctx, recordingID, callback) {
    MeetupsAPI.Bbb.deleteRecordingsURL(ctx, recordingID, function(err, deleteRecordings) {
