...
var getMeetingInfoURL = module.exports.getMeetingInfoURL = function(ctx, meetingProfile, callback) {
    ...
    var bbbConfig = _getBBBConfig(ctx.tenant().alias);
...
var joinURL = module.exports.joinURL = function(ctx, meetingProfile, configXML, host, protocol, callback) {
    // Obtain the configuration parameters for the current tenant
    var bbbConfig = _getBBBConfig(ctx.tenant().alias);
    ...
    BBBProxy.executeBBBCall(meetingInfoURL, function(err, meetingInfo) {
        ...
        if ( meetingInfo.returncode === 'FAILED' && meetingInfo.messageKey === 'notFound' ) {
            ...
            if (meetingProfile.resourceType === 'group') {
                ...
                params['meta_bn-recording-ready-url'] = protocol+'://'+host+'/api/meetup/'+meetingProfile.id+'/recording';
            }
            ...
            BBBProxy.executeBBBCall(createMeetingURL, function(err, meetingInfo) {
                ...
                MeetingsDAO.createMeetup(meetingID, ctx.user().id, ctx.user().displayName, record, null);
...
var endURL = module.exports.endURL = function(ctx, meetingProfile, callback) {
    // Obtain the configuration parameters for the current tenant
    var bbbConfig = _getBBBConfig(ctx.tenant().alias);
    ...
    BBBProxy.executeBBBCall(meetingInfoURL, function(err, meetingInfo) {
        ...
        if ( meetingInfo.returncode === 'FAILED' && meetingInfo.messageKey === 'notFound' ) {
            return callback(null, {'returncode':'failed','response':'Meetup already ended' } );
...
var getRecordingsURL = module.exports.getRecordingsURL = function(ctx, profile, callback) {
    ...
    MeetingsDAO.getMeetup(meetingID, function (err, meeting){
        if (err) {
            return callback(err);
        } else if (!meeting) {
...
var deleteRecordingsURL = module.exports.deleteRecordingsURL = function(ctx, recordingID, callback) {
    // Obtain the configuration parameters for the current tenant
    var bbbConfig = _getBBBConfig(ctx.tenant().alias);
